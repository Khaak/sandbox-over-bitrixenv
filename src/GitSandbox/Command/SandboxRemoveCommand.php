...
class SandboxRemoveCommand extends Command {
	protected function configure() {
		...
		$this
			...
			->setDescription($mess["SandboxRemoveCommand"]["Description"])
			->setHelp($mess["SandboxRemoveCommand"]["Help"])
			->addArgument(
				'project',
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		$project_path_full = '/home/' . $settings["PROJECT_USER"] . '/ext_www/' . $projectname;
		$fs = new Filesystem();
		if (!$fs->exists($project_path_full)) {
			throw new \Exception('Project ' . $projectname . ' not exists (' . $project_path_full . ')');
		}
		$sandbox_path = "/home/" . $user . "/ext_www/" . $projectname;
