...
class VirtualhostRemoveCommand extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		$nginxconffile1 = $settings["NGINX_CONF_DIR"] . "/bx/site_avaliable/bx_ext_" . $host . ".conf";
		$nginxconffile2 = $settings["NGINX_CONF_DIR"] . "/bx/site_enabled/bx_ext_" . $host . ".conf";
		$nginxsslconffile1 = $settings["NGINX_CONF_DIR"] . "/bx/site_avaliable/bx_ext_ssl_" . $host . ".conf";
		$nginxsslconffile2 = $settings["NGINX_CONF_DIR"] . "/bx/site_enabled/bx_ext_ssl_" . $host . ".conf";
		$httpdconffile = $settings["HTTPD_CONF_DIR"] . "/bx/conf/bx_ext_" . $host . ".conf";
		...
		$fs->remove($nginxsslconffile1);
		$fs->remove($nginxsslconffile2);
