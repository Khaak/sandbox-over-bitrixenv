...
class VirtualhostAddCommand extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		$project_path = "/home/" . $settings["PROJECT_USER"] . "/ext_www";
		$project_path_full = $project_path . "/" . $projectname;
		$fs = new Filesystem();
		...
		if ($type == "sandbox") {
			...
			$vhost_root = "/home/" . $user . "/ext_www/" . $projectname;
		}
		...
		$nginxssltemplatefile = $settings["TEMPLATES_DIR"] . "/" . $type . "/nginx/site_template_ssl.conf";
		$httpdtemplatefile = $settings["TEMPLATES_DIR"] . "/" . $type . "/httpd/site_template.conf";
		$nginxconffile = $settings["NGINX_CONF_DIR"] . "/bx/site_avaliable/bx_ext_" . $vhost_name . ".conf";
		$nginxsslconffile = $settings["NGINX_CONF_DIR"] . "/bx/site_avaliable/bx_ext_ssl_" . $vhost_name . ".conf";
		$httpdconffile = $settings["HTTPD_CONF_DIR"] . "/bx/conf/bx_ext_" . $vhost_name . ".conf";
		$projecthttpdconffile = $settings["HTTPD_CONF_DIR"] . "/bx/conf/bx_ext_" . $project_vhost_name . ".conf";
		if ($type == "sandbox") {
		...
		$nginxssltemplate = file_get_contents($nginxssltemplatefile);
		...
		$nginxssltemplate = str_replace($tags, $set, $nginxssltemplate);
		$fs->dumpFile($nginxconffile, $nginxtemplate);
		$fs->dumpFile($nginxsslconffile, $nginxssltemplate);
		unset($nginxtemplate);
		unset($nginxssltemplate);
		$fs->symlink($nginxconffile, $settings["NGINX_CONF_DIR"] . "/bx/site_enabled/bx_ext_" . $vhost_name . ".conf");
		$fs->symlink($nginxsslconffile, $settings["NGINX_CONF_DIR"] . "/bx/site_enabled/bx_ext_ssl_" . $vhost_name . ".conf");
